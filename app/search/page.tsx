...
interface SearchPageProps {
  searchParams: Promise<{ q?: string; s?: string; from?: string; to?: string }>;
}
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const [results, popularArticles, recentArticles] = await Promise.all([
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 font-mukta">
      ...
      {/* Search Header Box */}
      <div className="my-4 p-6 bg-white border border-slate-200 rounded-md shadow-xs">
        ...
        <form action="/search" method="GET" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3">
          <div className="lg:col-span-3">
            <input
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-sawal-red"
            />
          </div>
          ...
          <div className="lg:col-span-2">
            <button
              type="submit"
              className="w-full py-2.5 bg-sawal-red hover:bg-sawal-darkred text-white text-sm font-bold rounded transition-colors shadow-xs"
            >
              Search
            </button>
          </div>
        ...
        {query && (
          <p className="text-xs text-slate-500 mt-3 font-medium">
            &ldquo;{query}&rdquo; का लागि कुल {toNepaliDigits(results.length)} वटा सामग्री फेला परे
          </p>
        )}
      </div>
          ...
          {results.length > 0 ? (
            <div className="space-y-4">
              {results.map((art) => (
                <article
                  key={art.id}
                  className="p-4 bg-white border border-slate-200 rounded-md shadow-xs hover:shadow-md transition-shadow group flex flex-col sm:flex-row gap-4"
                >
                  <Link
                    href={`/news/${art.slug}`}
                    className="w-full sm:w-48 aspect-[16/10] rounded overflow-hidden bg-slate-100 flex-shrink-0 relative"
                  >
            ...
            <div className="py-16 text-center bg-white border border-slate-200 rounded-md p-8">
            ...
            <div className="py-16 text-center bg-white border border-slate-200 rounded-md p-8 text-slate-500 text-sm">
